...
import { API_BASE_URL, FEATURE_FLAGS, WHATSAPP_BASE_URL } from "../config/env";
...
const Property = () => {
    ...
    const loadProperty = async () => {
        try {
            const response = await fetch(`${API_BASE_URL}/api/allProperty`);
    ...
    const loadPaidProperties = async () => {
        ...
        try {
            const response = await fetch(`${API_BASE_URL}/api/userPayments/${userId}`);
    ...
    const handleWhatsApp = (contact) => {
        ...
        window.open(`${WHATSAPP_BASE_URL}/${contact}?text=${encodedMessage}`, '_blank');
    }
    ...
    const handlePayment = async (propId) => {
        ...
        if (!FEATURE_FLAGS.payments) {
            alert('Payments are currently unavailable.');
            return;
        }
        ...
        try {
            const response = await fetch(`${API_BASE_URL}/api/recordPayment`, {
                method: 'POST',
                headers: {
            ...
            <div className="property-container">
                {property.map((prop) => (
                    ...
                    >
                        ...
                        <img 
                            src={`${API_BASE_URL}/api/product/${prop.id}/image`} 
                            alt="Property" 
